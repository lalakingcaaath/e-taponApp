...
class _CampusState extends State<Campus> {
  ...
  void initState() {
    ...
    _database.ref().child('ITECH/bin2').onValue.listen((event) async {
      var value = event.snapshot.value;
      if(value != null) {
    ...
    _database.ref().child('ITECH/bin3').onValue.listen((event) async {
      var value = event.snapshot.value;
      if(value != null) {
    ...
    _database.ref().child('ITECH/bin4').onValue.listen((event) async {
      var value = event.snapshot.value;
      if(value != null) {
    ...
    _database.ref().child('ITECH/bin5').onValue.listen((event) async {
      var value = event.snapshot.value;
      if(value != null) {
    ...
    _database.ref().child('ITECH/bin6').onValue.listen((event) async {
      var value = event.snapshot.value;
      if(value != null) {
  ...
  void _showNotification() async {
    ...
    if(_bin1*100 == firstBinThreshold){
      ...
      message = 'Garbage Bin 1 is now ${_bin2*100}% full!';
    }
